...
export const UserBaseSchema = z.object({
  ...
  nickname: z.string().min(1, 'ニックネームは必須です').max(50, 'ニックネームは50文字以内です'),
  email: z.string().email('有効なメールアドレスを入力してください'),
  role: UserRoleSchema,
  storeId: z.string().min(1, '店舗IDは必須です'),
  color: z.string().optional(),
  hourlyWage: z.number().positive('時給は正の数値である必要があります').optional(),
...
export const ShiftBaseSchema = z.object({
  ...
  storeId: z.string().min(1, '店舗IDは必須です'),
  shiftDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, '日付形式はYYYY-MM-DDです'),
  startTime: z.string().regex(/^\d{2}:\d{2}$/, '時刻形式はHH:MMです'),
  endTime: z.string().regex(/^\d{2}:\d{2}$/, '時刻形式はHH:MMです'),
...
export const StoreConfigSchema = z.object({
  ...
  businessHours: z.object({
    start: z.string().regex(/^\d{2}:\d{2}$/, '時刻形式はHH:MMです'),
    end: z.string().regex(/^\d{2}:\d{2}$/, '時刻形式はHH:MMです'),
  }),
