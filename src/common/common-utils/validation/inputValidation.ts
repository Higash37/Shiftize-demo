 ...
 *
 * 【switch文の解説】
 * value を各 case と比較し、一致する case のブロックを実行する。
 * default → どの case にも一致しない場合に実行される。
 *
 * @param value - 検証する値
 * @param type - フィールドの種類
 ...
 *
 * 【Object.entries() の解説】
 * オブジェクトのキーと値のペアを配列にして返す。
 * { name: rule1, email: rule2 } → [["name", rule1], ["email", rule2]]
 * for...of と組み合わせて全フィールドを反復処理する。
 *
 * 【使い方の例】
 * ```typescript
 * const result = validateForm({
 *   email: { value: "test@example.com", type: "email", required: true },
 *   password: { value: "Pass123", type: "password", required: true },
 * });
 * // result.isValid → true/false
 * // result.errors → { password: "パスワードは8文字以上..." }
 * ```
 *
