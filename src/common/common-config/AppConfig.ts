...
const getExpoConfig = (): ExpoConfigType => {
  const devProjectId = getEnvVar(
    "EXPO_PUBLIC_DEV_PROJECT_ID",
    false,
    "default-dev-project"
  );
  const prodProjectId = getEnvVar(
    "EXPO_PUBLIC_PROD_PROJECT_ID",
    false,
...
const getEmailConfig = (): EmailConfigType => {
  ...
  const port = Number.parseInt(getEnvVar("EMAIL_PORT", false, "587"), 10);
...
const initializeAppConfig = (): AppConfigType => {
  try {
    ...
    try {
      ...
      console.warn("Security log event failed", loggingError);
      // ログ記録の失敗は無視
    }
    ...
    try {
      SecurityLogger.logEvent({
        type: "system_error",
        userId: "system",
        details: `App configuration initialization failed: ${
 ...
 * Firebase設定の取得
 */
export const getFirebaseConfigFromApp = (): FirebaseConfigType =>
  AppConfig.firebase;
...
export const validateConfig = (): { isValid: boolean; errors: string[] } => {
  ...
  try {
    ...
    // セキュリティ設定の検証
    if (
      AppConfig.security.maxRetryAttempts < 1 ||
      AppConfig.security.maxRetryAttempts > 10
    ) {
      errors.push("Invalid max retry attempts (must be 1-10)");
    }

    if (
      AppConfig.security.tokenExpiryDays < 1 ||
      AppConfig.security.tokenExpiryDays > 365
    ) {
...
export interface ConfigSummary {
  ...
  security: AppConfigType["security"];
  notifications: AppConfigType["notifications"];
  shifts: AppConfigType["shifts"];
}
...
export const getConfigSummary = (): ConfigSummary => {
  const summary: ConfigSummary = {
    ...
    firebase: {
      ...
      apiKey: AppConfig.isProduction
        ? "[HIDDEN]"
        : AppConfig.firebase.apiKey.substring(0, 10) + "...",
    },
