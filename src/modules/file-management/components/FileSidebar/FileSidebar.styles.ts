...
const getSidebarWidth = () => {
  if (isMobile) return width < 480 ? 200 : 250; // 小さいモバイルは200px、通常モバイルは250px
  if (isTablet) return 280; // タブレットは280px
  return 300; // デスクトップは300px
};
...
export const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: isMobile ? (width < 480 ? 4 : 8) : 16,
    paddingVertical: 12,
    borderBottomWidth: 1,
    borderBottomColor: "#e9ecef",
    backgroundColor: "#f8f9fa",
    gap: isMobile ? 4 : 12, // ホームボタンとプラスボタンの間隔
  },
  ...
  homeText: {
    fontSize: isMobile ? (width < 480 ? 9 : 11) : isTablet ? 16 : 18,
    fontWeight: "600",
