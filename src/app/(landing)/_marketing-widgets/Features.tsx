  ...
  const managerFeatures = [
    {
      ...
      title: "直感的なガントチャート",
      description: "ドラッグ&ドロップでシフト作成。一目で人員配置を確認でき、Excelより圧倒的に早く作業完了。",
      highlight: "月10時間削減"
    },
    {
      icon: (
      ...
      title: "自動計算・分析",
      description: "人件費自動計算、シフト充足率の可視化、月次レポート自動生成で経営判断をサポート。",
      highlight: "コスト15%削減"
    },
    {
      icon: (
      ...
      title: "募集シフト機能",
      description: "急な欠勤時の穴埋め募集。スタッフへの一斉通知、応募状況の管理で緊急対応も安心。",
      highlight: "対応時間90%短縮"
    },
    {
      icon: (
      ...
      title: "外部連携",
      description: "Googleカレンダー自動同期、LINE通知、給与計算ソフト連携で業務を一元化。",
      highlight: "連携拡大中"
    }
  ];
  ...
  const staffFeatures = [
    {
      ...
      title: "簡単シフト申請",
      description: "スマホから希望提出、複数日程を一括申請、過去の申請履歴確認も可能。",
      highlight: "申請時間70%短縮"
    },
    {
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-5 5v-5zM4.828 4.828A4 4 0 015.656 4H20a2 2 0 012 2v12a2 2 0 01-2 2h-5L9 14H4a2 2 0 01-2-2V6a2 2 0 012-2h1.172z" />
        </svg>
      ),
      title: "リアルタイム確認",
      description: "確定シフトの即時確認、カレンダー表示対応、他スタッフのシフトも確認可能。",
      highlight: "即座に同期"
    },
    {
      icon: (
      ...
      title: "変更通知",
      description: "シフト変更の即時通知、募集シフトへの応募、リマインダー機能で見落とし防止。",
      highlight: "通知100%到達"
    },
    {
      icon: (
      ...
      title: "ファイル管理",
      description: "マニュアルや書類にいつでもアクセス、必要な情報をスマホで確認。",
      highlight: "24時間アクセス"
    }
  ];

  const statusFlow = [
    { status: "申請中", color: "yellow", description: "スタッフがシフト希望を提出" },
    { status: "承認済み", color: "blue", description: "管理者が承認した確定シフト" },
    ...
    <section id="features" className="section-padding bg-gradient-to-b from-gray-50 to-white">
      <div className="container-responsive">
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-16">
          {(activeTab === 'manager' ? managerFeatures : staffFeatures).map((feature, index) => (
              ...
              return (
                <React.Fragment key={index}>
                  <div className="text-center group">
                    <div className={`px-4 py-2 rounded-xl border-2 font-semibold text-sm mb-2 group-hover:scale-105 transition-transform ${colorClasses[item.color]}`}>
                      {item.status}
                    </div>
                    <p className="text-xs text-gray-500 max-w-20">{item.description}</p>
                  </div>
