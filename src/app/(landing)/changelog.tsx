...
export default function ChangelogPage() {
  ...
  const changelog = [
    {
      ...
      type: 'major',
      title: '分割レイアウト実装完了',
      description: 'タブレット・モバイル向けカレンダー + 1日ガントチャート分割表示を実装',
      changes: [
    ...
    },
    {
      ...
      type: 'minor',
      title: 'Firebase最適化完了',
      description: 'アーキテクチャ決定によりFirebase継続、パフォーマンス最適化を実施',
      changes: [
        'Firebase最適化路線の決定',
        'バックエンド移行計画の中止',
        '現行システムの改善・拡張に注力',
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'released': return { icon: 'check-circle', color: '#10B981' };
      case 'in-progress': return { icon: 'hourglass-empty', color: '#F59E0B' };
      case 'planned': return { icon: 'schedule', color: '#6B7280' };
      default: return { icon: 'help', color: '#6B7280' };
    }
  };

  const getStatusText = (status: string) => {
    switch (status) {
      case 'released': return 'リリース済み';
      case 'in-progress': return '開発中';
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.filterScroll}>
            {categories.map((category) => (
              <TouchableOpacity
                ...
                style={[
                  styles.filterButton,
                  selectedCategory === category.id && [styles.filterButtonActive, { borderColor: category.color }]
                ]}
