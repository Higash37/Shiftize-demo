 ...
 * 募集シフト通知をLINEグループに送信
 */
export const sendRecruitmentNotification = functions
  .region('asia-northeast1')
  .https.onCall(async (data: {
    storeId: string;
    recruitmentShift: {
    ...
    if (!userDoc.exists || userDoc.data()?.role !== 'master') {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Only master users can send recruitment notifications'
      );
    }

    // 🔒 店舗アクセス権限チェック
    const userData = userDoc.data();
    if (userData?.storeId !== data.storeId) {
      throw new functions.https.HttpsError(
        'permission-denied',
    ...
    // 募集シフト通知メッセージを生成
    const message = createRecruitmentMessage({
      ...data.recruitmentShift,
      storeId: data.storeId
    });

    try {
      // LINE Messaging API経由で送信（友達全員に配信）
      ...
      console.log('✅ LINE notification sent:', response.status);
 ...
 * 募集シフト通知メッセージを生成
 */
function createRecruitmentMessage(shift: {
  id: string;
  storeId: string;
  date: string;
  startTime: string;
  ...
  const formatDate = (dateStr: string): string => {
    ...
    // YYYY-MM-DD 形式の場合（多くの場合これが原因）
    if (/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) {
      // ISO形式として扱う（タイムゾーンの問題を避けるため）
  ...
  // 🔗 LINE Bot用URL (本番環境)
  const applicationUrl = `https://shift-scheduler-app-vu6i.onrender.com/user/shifts/recruitment?storeId=${shift.storeId}&shiftIds=${shift.id}`;

  return {
    type: 'flex',
    altText: `📋 シフト募集: ${formatDate(shift.date)} ${shift.startTime}-${shift.endTime}`,
    contents: {
      type: 'bubble',
      styles: {
        header: {
 ...
 * 一括募集シフト通知をLINEで送信
 */
export const sendBulkRecruitmentNotification = functions
  .region('asia-northeast1')
  .https.onCall(async (data: {
    storeId: string;
    recruitmentShifts: Array<{
      id: string;
      date: string;
      startTime: string;
      endTime: string;
    ...
    if (!userDoc.exists || userDoc.data()?.role !== 'master') {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Only master users can send bulk notifications'
      );
    }
    ...
    if (data.recruitmentShifts.length === 0) {
      throw new functions.https.HttpsError(
        'invalid-argument',
        'No recruitment shifts to notify'
      );
    }

    try {
      // 複数シフト用メッセージを生成
      const message = createBulkRecruitmentMessage({
        shifts: data.recruitmentShifts,
      ...
      console.log('✅ Bulk LINE notification sent:', response.status);

      return {
        success: true,
        messageId: response.data.messageId,
        shiftsCount: data.recruitmentShifts.length,
        sentAt: new Date().toISOString()
      };

    } catch (error: any) {
      console.error('❌ Failed to send bulk LINE notification:', error.response?.data || error.message);
      throw new functions.https.HttpsError(
        'internal',
        'Failed to send bulk LINE notification'
      );
    }
  });
 ...
 * 一括募集シフト通知メッセージを生成
 */
function createBulkRecruitmentMessage(data: {
  shifts: Array<{
    id: string;
    date: string;
    startTime: string;
  ...
  // 🔗 ベースURL (本番環境)
  const baseUrl = 'https://shift-scheduler-app-vu6i.onrender.com';

  // シフト一覧を構築
  const shiftItems = data.shifts.map(shift => ({
    type: 'box' as const,
  ...
  return {
    type: 'flex',
    altText: `📋 シフト募集: ${data.shifts.length}件の募集があります`,
    contents: {
      type: 'bubble',
      styles: {
        header: {
      ...
      header: {
        ...
        contents: [
          ...
          {
            type: 'text',
            text: `${data.shifts.length}件の募集があります`,
            color: '#ffffff',
            size: 'sm',
            align: 'center',
            margin: 'xs'
      ...
      body: {
        ...
        contents: [
          ...
          // シフト一覧を追加
          ...shiftItems.slice(0, 5), // 最大5件まで表示
          ...(data.shifts.length > 5 ? [{
            type: 'text' as const,
            text: `... 他 ${data.shifts.length - 5} 件`,
            size: 'xs' as const,
            color: '#666666',
        ...
        contents: [
          {
            ...
            action: {
              type: 'uri',
              label: 'アプリでシフトを確認',
              uri: `${baseUrl}/user/shifts/recruitment?storeId=${data.storeId}&shiftIds=${data.shifts.map(s => s.id).join(',')}`
            }
          },
          {
            type: 'text',
